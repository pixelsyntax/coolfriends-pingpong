	...
	public class Creature : MonoBehaviour {
		...
		void FixedUpdate() {
			...
			if (hitCount > 0) {
				...
				if (diff < 0f) { Rbody.MovePosition(Rbody.position + Vector3.down * diff * 0.5f); }
				else if (diff > 0f && diff < legDepth && wasOnFloor) { Rbody.MovePosition(Rbody.position + Vector3.down * diff * 0.8f); OnFloor = true; }
			}
