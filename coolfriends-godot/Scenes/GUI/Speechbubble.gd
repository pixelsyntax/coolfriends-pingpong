...
func _ready():
	...
	#control.remove_and_skip()
	control.get_parent().remove_child(control)
	viewport.add_child(control) # so i can edit the Control with Preview
	get_node("Area").connect("input_event", self, "_on_area_input_event")
	var quad = get_node("Area/Quad")
  
####

func setText(text):
	var label = viewport.get_node("Control/Panel/Label")
	label.text = text